...
if ~isempty(Sim)
    ...
    for j = 1:n_y
        ...
        % Plot noise
        lh(2) = fill([Sim.t(1:end); Sim.t(end:-1:1)],...
        ...
        plot(Sim.t,Sim.mFine(:,j) - sqrt(Sim.CFine(:,j,j)+Sim.Sigma_C(:,j,j)),'-',...
              'linewidth',options.sim.bound_lw,...
