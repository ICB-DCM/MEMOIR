...
if ~isempty(Sim)
    ...
    for j = 1:n_y
        ...
        % Plot noise
        lh(1) = fill([Data.PA.time(1:end);Data.PA.time(end:-1:1)],...
             [Sim.m(1:end,j) - Data.PA.Sigma_m(1:end,j);...
              Sim.m(end:-1:1,j) + Data.PA.Sigma_m(end:-1:1,j)],...
              options.sim.area_col);  
        hold on;
        plot(Data.PA.time, Sim.m(:,j) - Data.PA.Sigma_m(:,j),'-',...
